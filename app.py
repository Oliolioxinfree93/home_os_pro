...
def get_oauth_component():
    try:
        ...
        st.error(f"Missing secret: {e}. Check your [auth] section in Streamlit Secrets.")
        st.stop()
...
def check_login():
    ...
    with col2:
        ...
        **What you'll track:**
        ...
        st.markdown("---")
        if st.button("🛠️ Dev Mode (Skip Login)", use_container_width=True):
    ...
    try:
        ...
        if existing.data:
            ...
            supabase.table("budget_settings").update({
                "current_spent": current['current_spent'] + price
            }).eq("user_id", user_id).execute()
        else:
            supabase.table("budget_settings").insert({
                "user_id": user_id, "period": "monthly",
                "budget_limit": 500.0, "current_spent": price,
...
def db_get_budget():
    try:
        ...
        supabase.table("budget_settings").insert({
            "user_id": user_id, "period": "monthly",
            "budget_limit": 500.0, "current_spent": 0,
            "start_date": date.today().isoformat()
        }).execute()
...
def db_calculate_savings():
    try:
        ...
        waste_prevention = sum([r['price'] for r in consumed.data if r.get('price')]) if consumed.data else 0
...
def db_get_meal_plan(start_date):
    try:
        ...
        r = supabase.table("meal_plan").select("*").eq("user_id", user_id).gte("date", start_date.isoformat()).lte("date", end_date.isoformat()).execute()
        week_plan = {}
...
def db_consume_ingredients(ingredient_names):
    ...
    for ingredient in ingredient_names:
        try:
            r = supabase.table("inventory").select("*").eq("user_id", user_id).eq("status", "In Stock").ilike("item_name", f"%{ingredient}%").order("expiry_date").limit(1).execute()
            if r.data:
                item = r.data[0]
                if item['quantity'] > 1:
            ...
            report.append(f"Error with '{ingredient}': {e}")
...
with tab0:
    ...
    with c1:
        ...
        st.caption("Saved on delivery fees")
...
with tab5:
    ...
    with c1:
        ...
        st.metric("Spent This Month", f"${spent:.2f}", delta=f"${remaining:.2f} remaining")
        st.progress(min(pct / 100, 1.0))
        with st.expander("⚙️ Update Budget"):
            with st.form("budget_form"):
                new_budget = st.number_input("New Monthly Budget ($)", 0.0, 10000.0, limit)
                if st.form_submit_button("Update"):
                    supabase.table("budget_settings").update({
                        "budget_limit": new_budget, "current_spent": 0
                    }).eq("user_id", user_id).execute()
    ...
    with c2:
        ...
        try:
            ...
            if ph.data:
                ...
                cat_spend = df_ph.groupby('category')['price'].sum().reset_index()
                import plotly.express as px
                fig = px.pie(cat_spend, values='price', names='category')
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.info("Add purchases to see spending breakdown.")
        except:
